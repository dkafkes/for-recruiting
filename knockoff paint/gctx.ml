...
type thickness = {width: int}

let thick : thickness = {width = 10}
let normal : thickness = {width = 1}
...
let set_graphics_state (gc: gctx) : unit =
  ...
  let w = gc.thickness in Graphics.set_line_width w.width
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  begin match local_coords g p1 with
  | (x, y) -> set_graphics_state g; Graphics.draw_rect x y w h;
  end
...
let draw_ellipse (g: gctx) (p: position) (rx: int) (ry: int) : unit =
  begin match local_coords g p with
  | (x, y) -> set_graphics_state g; Graphics.draw_ellipse x y rx ry;
  end
...
let draw_points (g: gctx) (plist: position list) : unit =
  set_graphics_state g;
  List.iter (fun a -> draw_point g a) plist
