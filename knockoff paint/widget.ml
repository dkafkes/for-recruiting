...
let make_control (v: 'a) : 'a value_controller =
  ...
  {add_change_listener =
     (fun (new_listener: 'a -> unit)-> listeners.contents <- new_listener :: listeners.contents);
   get_value= (fun() -> values.contents); 
   change_value = (fun x -> (values.contents <- x; List.iter (fun f-> f x) listeners.contents))}
...
let checkbox (init: bool) (s: string)
  ...
  let (w, lc) = label s in
  let mc = make_control init in  

  let new_widget =
    {repaint = (fun (g: Gctx.gctx) ->
         let w2 = {repaint = (fun (g: Gctx.gctx) -> Gctx.draw_line g (1, 18) (1,1);
                               Gctx.draw_line g (1,1) (18, 1); Gctx.draw_line g (18,1) (18, 18);
                               Gctx.draw_line g (18, 18) (1, 18);
                               (if mc.get_value() = true then (Gctx.draw_line g (1, 18) (18, 1); Gctx.draw_line g (1,1) (18,18);)));
                   handle = (fun _ _ -> ());
                   size = (fun _ -> (fst(w.size())+20), snd(w.size()))} in
         let this= hpair w w2 in this.repaint g);
     handle = (fun g e-> if Gctx.event_type e = Gctx.MouseDown then mc.change_value (not (mc.get_value())));
     size = (fun _ -> (fst(w.size())+20), snd(w.size()))}

  in (border new_widget, mc)
...
let radio_button (vc: string value_controller) (s: string) : widget * label_controller =
  let (w, lc) = label s in
  ...
  let new_widget =
    {repaint = (fun (g: Gctx.gctx) ->
         let w2 = {repaint = (fun (g: Gctx.gctx) -> Gctx.draw_line g (1, 18) (1,1);
                               Gctx.draw_line g (1,1) (18, 1); Gctx.draw_line g (18,1) (18, 18);
                               Gctx.draw_line g (18, 18) (1, 18);
                               (if vc.get_value() = s then (Gctx.fill_rect g (1,18) (17,17);)));
                   handle = (fun _ _ -> ());
                   size = (fun _ -> (fst(w.size())+20), snd(w.size()))} in
         let this= hpair w w2 in this.repaint g);
     handle = (fun g e-> if Gctx.event_type e = Gctx.MouseDown then vc.change_value s);
     size = (fun _ -> (fst(w.size())+20), snd(w.size()))}

  in (border new_widget, lc)

let cluster (make_this: string list) (vc: string value_controller) : widget =

  let rec combined (make_this: string list) : widget list =
    begin match make_this with
      | [] -> []
      | [h] -> let (r, lc) = radio_button vc h in r :: []
      | x :: xs -> let (r, lc) = radio_button vc x in r :: combined xs
    end in
  let clustered = combined make_this in
  hlist clustered